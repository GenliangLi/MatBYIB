    ...
    methods
        function plot_t_fig(obj,t,ht,ty)
            ...
            if ty==1
                ...
                ylabel('h_t','FontSize',14,'Fontname','??¨¬?'); 
            else
                plot(t,ht,'k');hold on
                set(gca,'fontsize',12);
                set(gca,'LineWidth',1); 
        ...
        function plot_f_fig(obj,f,hf,ty)
            ...
            if ty==1
                ...
                set(get(gca,'xlabel'),'Fontsize',14);
                set(get(gca,'ylabel'),'Fontsize',14);
                xlabel('Frequency(Hz)','FontSize',14,'Fontname','??¨¬?'); 
                ylabel('h_f','FontSize',14,'Fontname','??¨¬?'); 
            else
                plot(f,hf,'k');hold on
                set(gca,'fontsize',12);
                set(gca,'LineWidth',1); 
        ...
        function [fig,ax]=cornerplot(obj,data, varargin)
            ...
            %   ax  : Axes handles
                    ...
                    for i = 1:nDims
                        ...
                        plot([mean_val+std_devs(i) mean_val+std_devs(i)], [0 max(h.Values)], 'b--', 'linewidth',0.5.*linewidth);hold on
                        plot([mean_val-std_devs(i) mean_val-std_devs(i)], [0 max(h.Values)], 'b--', 'linewidth',0.5.*linewidth);hold on
                        title(titles{i})
                        if i == nDims
                        ...
                        if ~isempty(truths)
                            hold on
                            plot([mean_val mean_val],[0 1], 'b--', 'linewidth',1*linewidth)
                        end
                        
                        if ~isempty(truths)
                            hold on
                            plot([truths(i) truths(i)], [0 1], 'r--', 'linewidth',1*linewidth)
                        end
                        ...
                        for d1 = 1:nDims-1
                            for d2 = d1+1:nDims
                                ...
                                if ~isempty(truths)
                                    ...
                                    plot(xl, [mean(data(:,d2)) mean(data(:,d2))],'b--', 'linewidth',1*linewidth)
                                    plot([mean(data(:,d1)) mean(data(:,d1))], yl,'b--', 'linewidth',1*linewidth)
                                    plot(xl, [truths(d2) truths(d2)],'r--', 'linewidth',1*linewidth)
                                    plot([truths(d1) truths(d1)], yl,'r--', 'linewidth',1*linewidth)
                                end
                                x_ticks = get(gca,'XTick');
                                x_tick_labels = arrayfun(@num2sci_str, x_ticks, 'UniformOutput', false);
                                set(gca,'XTickLabel', x_tick_labels);
            ...
            function formatted_title = format_title(obj,parameter, median, lower, upper)
                ...
                % Example output: "Parameter=1.23^{+0.45}_{-0.67}"
                        if isempty(median)
                            ...
                            try
                                median_str = scientific_format(obj,median);
                            catch
                                median_str = 'N/A';
                            end
                        end
                        if isempty(lower)
                            ...
                            lower_str = scientific_format(obj,lower);
                        end
                        if isempty(upper)
                            upper_str = 'N/A';
                        else
                            upper_str = scientific_format(obj,upper);
                        end
                        formatted_title = sprintf('%s=%s^{+%s}_{-%s}', parameter, median_str, upper_str, lower_str);
            end
            function formatted_str = scientific_format(obj,value)
                    formatted_str = sprintf("%.2e", value);
                    [base, exponent] = strsplit(formatted_str, 'e');
                    formatted_str = sprintf('%s¡Á10^{%s}', base{1}, base{2});
            end
            
            function sci_str = num2sci_str(obj,num)
                     sci_str = num2str(obj,num, '%e');
            end
                 
          
            function [fig,ax]=cornerplot_simple(obj,data, varargin)
            %CORNERPLOT Create corner plot for parameter distributions
            %
            % Inputs:
            %   data    : Parameter samples [Nsamples x Nparams]
            ...
            for i = 1:nDims
                ...
                ax(i+top_margin,i) = tight_subplot(obj,nDims+top_margin, nDims, i+top_margin, i, gutter, margins);
                edges = linspace(bounds(1,i), bounds(2,i), hist_bins);
                h = histogram(truncated_data(:,i), 'BinEdges', edges,'normalization', 'probability', 'displaystyle', 'stairs', 'edgecolor', 'k')
                set(gca,'xlim', bounds(:,i),'ylim', [0 max(h.Values)], axes_defaults,'ytick',[]);
            ...
            if nDims > 1
                for d1 = 1:nDims-1 % col
                    for d2 = d1+1:nDims % row
                        ...
                        % Compute 68% and 90% density levels
                        target_percentile = [0.68]; % 
                        level = quantile(density(:), target_percentile);
                        
                        % Compute 1¦Ò level from the density
                        % sigma_level = mean(density(:)) + std(density(:));
                        % 
                        % %Plot 68% and 90% contours
                        % contour(X, Y, density, [sigma_level sigma_level], ...
                        %     'LineColor', 'k', ...
                        %     'LineWidth', 0.7);hold on
                        contour(X, Y, density, [level(1) level(1)], ...
                            'LineColor', 'b', ...
                            'LineWidth', 0.7);hold on
                        % contour(X, Y, density, [level(2) level(2)], ...
                        %     'LineColor', 'r', ...
                        %     'LineWidth', 0.7);
                        % 
                        % Set axis limits and defaults
                        set(gca, 'xlim', bounds(:,d1), 'ylim', bounds(:,d2), axes_defaults);                                
                        
                        % Plot mean and truth lines
                        if ~isempty(truths)
                            yl = get(gca, 'ylim');
                            xl = get(gca, 'xlim');
                            hold on
